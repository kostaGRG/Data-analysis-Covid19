...
week = '2021-W'+string(start_week - 1);
rows = strcmp(data.Week,week);
previous_day = data(rows,{'PCR_Tests','Rapid_Tests'});

% Store the data from previous week's last day.
...
for i = start_week:final_week
    ...
    % In following rows we subtract current values of pcr and rapid with
    % the previous one.
    ...
    [CI, difference, greece_positivity_mean] = comparePositivityRates(greece_positivity_rates, eu_positivity_rate);
    ...
    fprintf('\nBootstrap 95 %% confidence intervals for greece''s positivity rates mean');
    fprintf('\n[%.3f%% %.3f%%]',CI(1),CI(2));
    ...
    if difference ~= 0
        ...
        fprintf('\nIn this week positivity rates of Greece and Europe are similar');
    end
...
legend('Greece''s positivity rate','Europe''s positivity rate','Greece''s positivity rate bootstrap 95% confidence intervals','Location','northwest');
